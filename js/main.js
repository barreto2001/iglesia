...
$('#aclogin').on('click',function(){ // MUESTRA EL FORM LOGIN AL HACER CLIC EN EL BOTON
...
$('#acregistro').on('click',function(){ // MUESTRA EL FORM REGISTRO AL HACER CLIC EN EL BOTON
	$('#login').hide();
	$('#registro').show();
});
...
$('#acom').on("keyup",function(){ // determina la cantidad de acompañantes y sillas disponibles
	...
	if (acompañante>disponible.length) { //verifica si hay sillas disponibles segun los acompañantes
		...
		alert('solo hay disponible '+disponible.length+' sillas');
		...
		for (var i = 2; i <= acompañante; i++) {
			...
			$('#micompañia').append('<input type="text" class="mas" id="name'+i+'" name="name'+n+'" placeholder="Nombres">');
			$('#micompañia').append('<select class="browser-default custom-select mas"  id="tipo'+i+'" name="tipo'+n+'" style="width:30%;"> <option value="" disabled selected>Tipo Documento</option> <option value="CC">Cedula de Ciudadania</option> <option value="TI">Tarjeta de Identidad</option><option value="CE">Cedula de Extranjeria</option></select>');
			$('#micompañia').append('<input type="number" class="mas" id="document'+i+'" name="documento'+n+'" placeholder="N° documento">');
